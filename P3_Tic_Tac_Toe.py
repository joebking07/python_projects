...
def game(min_num, max_num, max_attempts, score_bonus, level_name):
    ...
    while attempt < max_attempts:
        ...
        try:
            ...
            if choice == mystery_num:
                ...
                print(
                    f"Well done!! \u2713 you have win in {attempt} attempt , you have +{score}, record time: {elapsed_T:.2f} second"
                )
                history_list.append(
                    f" level: {level_name}; score: {score}; attempt before win {attempt}"
                )
    ...
    history_list.append(
        f"level: {level_name} ; score: {score}; attempt: {max_attempts}"
    )
    if len(history_list) > 3:
        history_list.pop(0)
    return -5
...
def principal():
    ...
    while True:
        ...
        level_input = (
            input(
                "enter your desired level between: easy / 1, normal / 2, difficult / 3: "
            )
            .lower()
            .strip()
        )
